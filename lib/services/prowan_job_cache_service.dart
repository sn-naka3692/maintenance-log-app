...
class ProwanJobCacheService {
  ...
  Future<ProwanJobCacheMatchResult?> findByScannedJobNumber(
    ...
    final candidates = findFuzzyJobNumberMatches(trimmed, allNumbers);
    if (candidates.isEmpty) return null;

    final best = candidates.first;
    final cache = await findByExactJobNumber(best.candidate);
    if (cache == null) return null;

    return ProwanJobCacheMatchResult(
      cache: cache,
      matchedJobNumber: best.candidate,
      distance: best.distance,
      isExactMatch: false,
      // 複数候補がある場合、UI側で選択肢として提示できるようにする
      alternativeCandidates: candidates.length > 1
          ? candidates.sublist(1).map((c) => c.candidate).toList()
          : [],
    );
  }
