...
class WorkReport {
  ...
  /// 【不具合修正・2026-09-01】「充填なし」表記が本一覧に含まれておらず、
  /// 冷媒充填していない案件(SE北海道大学病院、ラッキー菊水元町店など)が
  /// 一覧・検索・レポート集計で誤って「冷媒充填案件」として表示される
  /// 事故が発生した。本リストは今後も現場入力の表記ゆれ(スキャン結果や
  /// プルダウン選択肢の追加等)により同種の抜け漏れが再発しやすいため、
  /// 新しい「未充填」表記を追加する際は、必ずFirestore本番データの
  /// 実際の値パターンを確認してから追加すること(このリストに載って
  /// いない値は自動的に「充填あり」と判定される仕様のため)。
  /// 【重要・再発防止】冷媒種類はプルダウン化されており、選択肢は
  /// [RefrigerantTypeOptions.noneValues](本ファイル上部)で一元管理
  /// されている。プルダウンに新しい「未充填」表記を追加した場合は、
  /// 必ずこの[_notFilledTypeValues]にも同じ値を追加すること(2箇所を
  /// 同時に更新しないと、今回と同種の誤判定が再発する)。
