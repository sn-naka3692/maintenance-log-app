...
const List<FutureConsideration> futureConsiderations = [
  ...
  FutureConsideration(
    title: '月次データの自動CSVエクスポート・NAS保管(今後の課題)',
    description:
        'SE店舗・プロワン案件それぞれの日報データを、月ごとに自動でCSV形式に抽出・'
        '保管する仕組みの導入を検討中。ある程度データが蓄積した段階で、保管場所を'
        '会社のNASなどローカル環境に移す運用を想定している。\n\n'
        '技術的には、Firebase Admin SDKを用いて社内PC(NASにアクセス可能な常時稼働'
        '端末)からFirestoreのwork_reportsを月次で取得し、SE用・プロワン用に分けて'
        'CSVを生成、NASの共有フォルダへ直接出力するスクリプトをタスクスケジューラ等'
        'で定期実行する方式(社内PC経由方式)が最もシンプルで運用しやすいと考えている。\n\n'
        '実装にあたっては、(1) CSV化する項目の範囲(日報全項目か、読み取り機能に'
        '関わる項目のみか)、(2) 月の区切り基準(作業日か登録日か)、(3) NASへの保管'
        'を完全自動にするか一度人の確認を挟むか、(4) スクリプトを常駐させる社内PCの'
        '選定、(5) 実行結果(成功・失敗、件数)の通知要否、といった運用方針の確定が'
        '必要なため、中長期の検討課題として保留している。',
  ),
];
